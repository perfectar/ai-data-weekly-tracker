...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .week-card, .empty {{ background: var(--panel); border: 1px solid var(--line); border-radius: 8px; padding: 18px; box-shadow: var(--shadow); }}
    ...
    .tag {{ display: inline-flex; align-items: center; border-radius: 999px; background: var(--soft); color: var(--accent-dark); padding: 5px 9px; font-size: 12px; font-weight: 700; }}
    ol {{ margin: 0 0 16px; padding-left: 22px; color: var(--muted); }}
    li {{ margin: 6px 0; }}
    .button {{ display: inline-flex; align-items: center; justify-content: center; min-height: 40px; padding: 8px 13px; border-radius: 8px; background: var(--accent); color: #fff; font-weight: 750; }}
    ...
    @media (max-width: 760px) {{ .stats, .grid {{ grid-template-columns: 1fr; }} .wrap {{ padding: 22px 14px 42px; }} }}
  </style>
</head>
<body>
  <div class="wrap">
